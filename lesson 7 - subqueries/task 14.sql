...
WITH top_products AS ( SELECT product_id FROM products ORDER BY price DESC LIMIT 5 ), 
unnest AS ( SELECT order_id, product_ids, unnest(product_ids) AS product_id FROM orders ) 
