...
В результат включите четыре колонки: колонку с наименованиями кампаний, дату первого взаимодействия с приложением, количество дней, 
прошедших с даты первого взаимодействия (порядковый номер), и само значение Retention.
...
FROM   (SELECT user_id, min(time::date) over(partition by user_id) as start_date, time::date as date,  
               case when user_id in (8804, 9828, 9524, 9667, 9165, 10013, 9625, 8879, 9145, 8657,
                    ...
                    when user_id in (9752, 9510, 8893, 9196, 10038, 9839, 9820, 9064, 9930, 9529, 9267,
                                     ...
                                     9558, 8650, 9784, 8935, 9764, 8712) then 'Кампания № 2'
                    end as ads_campaign
        FROM   user_actions) t1
WHERE  ads_campaign is not null and (date - start_date) in (0, 1, 7)
GROUP BY ads_campaign, date, start_date
